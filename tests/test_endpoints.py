"""Endpoint smoke tests for v1 and v2 routers."""
import pytest
...
@pytest.mark.anyio
async def test_redirect_news(client):
    resp = await client.get("/news", follow_redirects=False)
    assert resp.status_code == 301
    assert "/v2/news" in resp.headers["location"]


@pytest.mark.anyio
async def test_redirect_match_preserves_params(client):
    resp = await client.get("/match?q=upcoming", follow_redirects=False)
    assert resp.status_code == 301
    assert "q=upcoming" in resp.headers["location"]
